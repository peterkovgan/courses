...
ggplot(data = rdu_flights, aes(x = dep_delay)) +  geom_histogram(binwidth = 15)
```
...
sfo_feb_flights %>% summarise(mean_ad = mean(arr_delay), max_ad = max(arr_delay),min_ad = min(arr_delay), sd_ad = sd(arr_delay), n = n())

sfo_feb_flights_arr_delayed <- sfo_feb_flights %>%  filter(arr_delay > 0 )
sfo_feb_flights_arr_delayed %>% summarise(n = n())
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(mean_dd1 = mean(arr_delay), median_dd1 = median(arr_delay), iqr_dd1 = IQR(arr_delay), sd_dd1 = sd(arr_delay), n1 = n())
```
...
nycflights %>%
  summarise(avg_speed = avg_speed, tailnum=tailnum) %>%
...
9.  Make a scatterplot of `avg_speed` vs. `distance`. Which of the
    ...
    There is an overall postive association between distance and average
    speed.
...
nycflights %>%  summarise(num_in_q = (sum(arr_type=="on time" & dep_type=="delayed")) / sum(dep_type=="delayed"))
