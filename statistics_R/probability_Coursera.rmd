...
kobe_basket[order(kobe_basket$game, kobe_basket$quarter,kobe_basket$time),]
```
...
kobe_streak %>%
  summarise(mean = mean(length), median = median(length), max = max(length), min = min(length), iqr = IQR(length), mode=getmode(length), n = n())

kobe_streak %>%
  summarise(prob0 = (sum(length==0))/n(), prob1 = (sum(length==1))/n(),prob2 = (sum(length==2))/n() ,prob3 = (sum(length==3))/n(),prob4 = (sum(length==4))/n())
