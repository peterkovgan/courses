...
However since we want the entries that are **not** `NA`s we can use the **Not** operator, `!`, to indicate "we want all the ones that are not NA":`!is.na()`. Hence we can `filter` out all non NAs in our `dplyr` chain:
